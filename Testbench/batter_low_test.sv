...
/// By default, it is expected to show high battery - Good battery
// Even though as value less than threshold is given for battery at reset it is assumed as the batter level is good
if(iDUT.iBUZZ.batt_low)
$fatal("Battery must be assumed to be good at right out of reset");
...
// Since low battery was given initially we expect battery low to have gone high after some clock cycles
if(!iDUT.iBUZZ.batt_low)
$display("ERROR !! Since low battery was given initially we expect battery low to have gone high after some clock cycles ");
...
/// Battery Low Check ///
batt = 12'h7ff; /// Value greater than the Threshold value
/// Wait Until Battery Low signal to the Piezo is asserted///
repeat (50000) @(posedge clk);
if(!iDUT.iBUZZ.batt_low)
$fatal("ERROR !!! Battery is not low as expected");

repeat (50000) @(posedge clk);
batt = 12'h801; /// Value less than the Threshold value
/// Wait Until Battery Low signal to the Piezo is de-asserted///
repeat (50000) @(posedge clk);
if(iDUT.iBUZZ.batt_low)
$fatal("ERROR !!! Battery is not high as expected");
repeat (50000) @(posedge clk);

/// Battery Low Check with Extreme Battery Value///
batt = 12'h0; /// Value greater than the Threshold value
/// Wait Until Battery Low signal to the Piezo is asserted///
repeat (50000) @(posedge clk);
if(!iDUT.iBUZZ.batt_low)
$fatal("ERROR !!! Battery is not low as expected");
...
repeat (50000) @(posedge clk);
batt = 12'hFFF; /// Value less than the threshold and the least value possible
/// Wait Until Battery Low signal to the Piezo is de-asserted///
repeat (50000) @(posedge clk);
if(iDUT.iBUZZ.batt_low)
$fatal("ERROR !!! Battery is not high as expected");
...
/////Segway still Should be in power up eben after all this/////
if(!iDUT.iAuth.pwr_up)
$fatal("ERROR !! Power Should not go down - No relation to Battery Low");
